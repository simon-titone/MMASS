function study2_cycle_power_ANOVA(v,NETW)
...
%% Power ANOVA
savepath = [v.homedir '/results/power/cycle/' ];
if ~exist(savepath); mkdir(savepath); end
% PSD = nan(v.nnetw, v.nbands,v.nsubj,v.stage,v.ncycles);
for st= 1:v.nstages
    for c = 1:v.ncycles
        for i = 1:v.nsubj
            s = v.subj(i);
...
for st= v.stage
    for w=1:v.nnetws
        for b=1:v.nbands
            vect1 = squeeze(PSD(w,b,st,1,:));
            vect2 = squeeze(PSD(w,b,st,2,:));
            vect3 = squeeze(PSD(w,b,st,3,:));
            ...
            ANOVA_p(w,b,st) =  p; ANOVA_F(w,b,st) =  f;
        end
    end
    clear w; clear ww;
    i = 1;
    pval_vect(:,:,st) = [ANOVA_p(1,1:v.nnetws,st) ANOVA_p(2,2:v.nnetws,st) ANOVA_p(3,3:v.nnetws,st) ANOVA_p(4,4:v.nnetws,st) ANOVA_p(5,5:v.nnetws,st)  ANOVA_p(6,6:v.nnetws,st)];
    F_anovanetw(:,:,st) = [ANOVA_F(1,1:v.nnetws,st) ANOVA_F(2,2:v.nnetws,st) ANOVA_F(3,3:v.nnetws,st) ANOVA_F(4,4:v.nnetws,st) ANOVA_F(5,5:v.nnetws,st)  ANOVA_F(6,6:v.nnetws,st)];
    clear ntw;
    [~, dummy2, ~, dummy4]=fdr_bh(pval_vect(:,:,st),v.fdr_thres,'pdep','no');
    crit_p(:,:,st) = dummy2;
    adj_p(:,:,st) = dummy4;
    adj_p_mat(1,1:6,st) = adj_p(1,1:6,st); adj_p_mat(2,2:6,st) = adj_p(1,7:11,st);
    adj_p_mat(3,3:6,st) = adj_p(1,12:15,st); adj_p_mat(4,4:6,st) = adj_p(1,16:18,st);
    adj_p_mat(5,5:6,st) = adj_p(1,19:20,st); adj_p_mat(6,6,st) = adj_p(1,21,st);
    
    clear dummy2; clear dummy4;
    index = find(pval_vect(i,:,st) == crit_p);
    ...
    [ivect,jvect]=find(abs(ANOVA_F(:,:,st))>=abs(crit_F));
    [ivect_unc,jvect_unc]=find(ANOVA_p(:,:,st)<=0.05);
    clear crit_F;
    ...
    saveas(gcf, [savepath filesep v.t_group '_cycle_power_ANOVA.png'])
    ...
    if v.violin == 1
        ...
        for w=1:v.nnetws
            for b=1:v.nbands
                if ANOVA_p(w,b,st) <0.05 == 1
                    x = [squeeze(PSD(w,b,st,1,:)); squeeze(PSD(w,b,st,2,:)); squeeze(PSD(w,b,st,3,:))];
                    ...
                    if v.fig_title == 1
                        title({[ v.bandname{b} ' ' v.stages{st} ' ' v.netwname{w}  ]}, 'FontSize', 16);
                        ylabel('Power', 'FontSize', 16);
                    end
                    if v.fig_labels == 1
                        f = num2str(round(ANOVA_F(w,b,st),3));
                        text(0.1, 0.98,['F = ' f] ,'Units','normalized');
                        p = round(ANOVA_p(w,b,st),4);
                        if p <0.001
                    ...
                    saveas(gcf, [ANOVA_violin filesep v.netwname{w} '_' v.bandname{b}   '_power_violin.png' ]);
                end
            end
        end
        close all;
