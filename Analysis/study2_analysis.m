...
v.homedir = '/Users/simontitone/GoogleDrive/PhD/Analysis/study2/';
v.vardir = [v.homedir 'vars']; v.var = [v.vardir '/v.mat'];
% load(v.var)

analysis = 's'; % c = cycle or s = stage

v.scatter = []; v.violin = [1]; v.fig_labels = []; v.fig_title = [1]; v.write = [];
v.out_cutoff = 3; v.plot_outliers = 1; v.logplot = 0;

%%%%% Excluded Subjects
study2_excl(v); load(v.var);
...
if strcmp(analysis ,'c')
    ...
    for g = 10:12
        ...
        % study2_cycle_ANOVA_3way(v, NETW)
        study2_cycle_ANOVA_ratio(v,NETW)
        ...
        % study2_cycle_power_ANOVA(v,NETW)
...
if strcmp(analysis ,'s')
    ...
    for g = 10
        ...
        % study2_stage_ANOVA_3way(v,NETW,design_matrix)
        study2_stage_ANOVA_ratio(v,NETW)
        % study2_conn_outliers(v, NETW);                              % Connectivity outliers
        % study2_within_level_means(v, NETW)                          % Within-level means
        % study2_within_level_stats(v, NETW)                          % Within-level statistics (tcdf needs R2018+ to run properly), not finished editing yet
        % study2_power_analysis(v);
        % study2_stage_power_ANOVA(v,NETW)
        % study2_power_ttest(v);
