function study3_stage_sess_av_ANOVA(v)
...
anova_dir = [v.homedir filesep 'results/ANOVA/interstage_av/'  ];
...
for i = 1:v.nsubj
  for b = 1:v.nbands
    for w=1:v.nnetws
        for ww=1:v.nnetws
            NREM2_av(w,ww,i,b) = mean([squeeze(NETW(w,ww,i,1,1,c,b)) squeeze(NETW(w,ww,i,1,2,c,b))]);
            NREM3_av(w,ww,i,b) = mean([squeeze(NETW(w,ww,i,2,1,c,b)) squeeze(NETW(w,ww,i,2,2,c,b))]);
            REM_av(w,ww,i,b) = mean([squeeze(NETW(w,ww,i,3,1,c,b)) squeeze(NETW(w,ww,i,3,2,c,b))]);
        end
    end
  end
end
...
for b = 1:v.nbands
    for w=1:v.nnetws
        for ww=1:v.nnetws
            NREM2 = squeeze(NREM2_av(w,ww,:,b));
            NREM3 = squeeze(NREM3_av(w,ww,:,b));
            REM = squeeze(REM_av(w,ww,:,b));
            nsubj = length(~isnan(NREM2));
            mat(:,1) = NREM2;mat(:,2) = NREM3;mat(:,3) = REM;
    ...
    if  v.fig_labels == 1
        title({  ['Inter-session Average Sleep Connectity'] [v.bandname{b}  ' Stage Effect' ' , n= ' num2str(nsubj) ]}); % generate corresponding rfx figure %
    else
        title({  ['Inter-session Average Sleep Connectity'] [v.bandname{b}  ' Stage Effect']}); % generate corresponding rfx figure %' , n= ' num2str(nsubj)
    ...
    saveas(gcf, [anova_dir v.bandname{b} '_interstage_av_ANOVA.png'])
    clear a2; clear i; clear mm; clear f

    %% Violin plots for each significant btw session comparison
    if v.violin == 1
        ANOVA_violin = [anova_dir   'interstage_violin'];
        ...
        for w=1:v.nnetws
            for ww=1:v.nnetws
                if ANOVA_p(w,ww,c,b) <0.05 == 1
                    ...
                    x = [squeeze(NREM2_av(w,ww,:,b)); squeeze(NREM3_av(w,ww,:,b)); squeeze(REM_av(w,ww,:,b))];
