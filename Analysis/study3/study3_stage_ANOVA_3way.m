function study3_stage_ANOVA_3way(v)
...
for s = sess
    anova_dir = [v.homedir filesep 'results/ANOVA/interstage/' v.sess{s} '_interstage_all3' filesep ];
    if ~exist(anova_dir); mkdir(anova_dir); end; cd(anova_dir)
    for c= 1
        for b = 6
            for w=1:v.nnetws
                for ww=1:v.nnetws
                    vect1 = squeeze(NETW(w,ww,:,1,s,c,b));
...
if v.excel ==1
    ...
    for c= 1
        for b = 1:v.nbands
            for w=1:v.nnetws
                for ww=1:v.nnetws
                    if ANOVA_p(w,ww,b) < 0.05 && ANOVA_p(w,ww,b) > 0
                        ANOVA_sig_F(w,ww,b) = ANOVA_F(w,ww,b);
                        ANOVA_sig_p(w,ww,b) = ANOVA_p(w,ww,b);
                    else
                        ANOVA_sig_F(w,ww,b) = NaN;
                        ANOVA_sig_p(w,ww,b) = NaN;
                    end
                end
            end
            ANOVA_sig_F(:,:,b) = triu(ANOVA_sig_F(:,:,b));
            ANOVA_sig_p(:,:,b) = triu(ANOVA_sig_p(:,:,b));
        end
    end
    ...
    for c = v.cycle
        for b = 1:v.nbands
            for w=1:v.nnetws
                for ww=1:v.nnetws
                    if  ANOVA_sig_F(w,ww,b) ~=0 && ~isnan(ANOVA_sig_F(w,ww,b)) && ANOVA_sig_p(w,ww,b)>0.001
                        result(w,ww,b) = {['F(1, ' num2str(v.nsubj-1)  ') = ' num2str(round(ANOVA_sig_F(w,ww,b),3)) ', p = ' num2str(round(ANOVA_sig_p(w,ww,b),3))]};
                    elseif  ANOVA_sig_F(w,ww,b) ~=0 && ~isnan(ANOVA_sig_F(w,ww,b)) && ANOVA_sig_p(w,ww,b)<0.001
                        result(w,ww,b) = {['F(1, ' num2str(v.nsubj-1)  ') = ' num2str(round(ANOVA_sig_F(w,ww,b),3)) ', p < 0.001']};
                    else
                        result(w,ww,b) = {'nan'};
                    end
                    if  ANOVA_sig_F(w,ww,b) ~=0 && ~isnan(ANOVA_sig_F(w,ww,b)) && adj_p_mat(w,ww,b)<0.05
                        result_corr(w,ww,b) = {['F(1, ' num2str(v.nsubj-1)  ') = ' num2str(round(ANOVA_sig_F(w,ww,b),3)) ', p = ' num2str(round(adj_p_mat(w,ww,b),3))]};
                    else
                        result_corr(w,ww,b) = {'nan'};
                    end
                end
            end
        end
