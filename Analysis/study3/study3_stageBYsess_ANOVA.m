function study3_stageBYsess_ANOVA(v)
%%%%% %NETW: network, network, subject, stage, session, cycle, band
%%%%% BANDS = 1-delta 2-theta 3-alpha 4-sigma 5-beta 6-gamma

% v.ANOVA = [1 3 4 6 7 9 10 11 12 13 14 16 17 21 24 27 28 29];
v.ANOVA = [1 3 4 6 7 9 10 11 12 13 14 16 17 18 21 24 27 28 29];
v.group = v.ANOVA;
...
anova_dir = [v.homedir filesep 'results/ANOVA/StageBySess_n' num2str(v.nsubj) '/'  ];
...
for c= 1
    for b = 1:v.nbands
        for w=1:v.nnetws
            for ww=1:v.nnetws
                ctrl_NREM2 = squeeze(NETW(w,ww,:,1,1,c,b));
                ctrl_NREM3 = squeeze(NETW(w,ww,:,2,1,c,b));
                ctrl_REM = squeeze(NETW(w,ww,:,3,1,c,b));
                exp_NREM2 = squeeze(NETW(w,ww,:,1,2,c,b));
                exp_NREM3 = squeeze(NETW(w,ww,:,2,2,c,b));
                exp_REM = squeeze(NETW(w,ww,:,3,2,c,b));
                nsubj = length(~isnan(ctrl_NREM2));
                mat(:,1,1) = ctrl_NREM2;mat(:,2,1) = ctrl_NREM3;mat(:,3,1) = ctrl_REM;
                mat(:,1,2) = exp_NREM2;mat(:,2,2) = exp_NREM3;mat(:,3,2) = exp_REM;
                [tbl,rm] = simple_mixed_anova(mat);
                f = table2array(tbl(7,4)); % (tbl(3,4)) = stage effect, 5,4 = session effect, 7,4 = stage x session
                p = table2array(tbl(7,5));
        ...
        if  v.fig_labels == 1
            title({  ['Session x Stage Effect'] [ v.bandname{b} ' , n= ' num2str(nsubj) ]}); % generate corresponding rfx figure %
        else
            title({  [v.bandname{b} ' Session x Stage Effect'  ]}); % generate corresponding rfx figure %' , n= ' num2str(nsubj)
        ...
        saveas(gcf, [anova_dir v.bandname{b} '_StageBySess.png'])
        clear a2; clear i; clear mm; clear f

        %         if v.scatter == 1
        %             intersess_scatter = [anova_dir filesep 'interstage_scatter'];
        ...
        if v.violin == 1
            ANOVA_violin = [anova_dir   'StageBySess_violin'];
...
%                     if b == 2 && w == 3 && ww == 3
                        ...
                        x = [c_NREM2; e_NREM2;c_NREM3;e_NREM3;c_REM;e_REM];
                        y = [repmat({'EXP NREM2'}, v.nsubj,1); repmat({'CTRL NREM2'}, v.nsubj,1) ; repmat({'EXP NREM3'}, v.nsubj,1); repmat({'CTRL NREM3'}, v.nsubj,1);repmat({'EXP REM'}, v.nsubj,1); repmat({'CTRL REM'}, v.nsubj,1)];
                        grouporder={ 'CTRL NREM2','EXP NREM2', 'CTRL NREM3', 'EXP NREM3','EXP REM', 'CTRL REM'};
...
%                         set(gcf, 'Position',  [10, 10, 350, 500]);
                        if  v.fig_labels == 1
                            title({ [ v.bandname{b} ' ' v.netwname{w} '-' v.netwname{ww} ' n = ' num2str(v.nsubj) ]}, 'FontSize', 16);
                        else
                            title({ [ v.bandname{b} ' ' v.netwname{w} '-' v.netwname{ww}]}, 'FontSize', 16);
                        end
                        if v.pval_labels == 1
                            f = num2str(round(ANOVA_F(w,ww,c,b),3));
                            text(0.1, 0.98,['F = ' f] ,'Units','normalized');
                        ...
                        saveas(gcf, [violin_band filesep '_' v.bandname{b} '_' v.netwname{w} '_' v.netwname{ww}   '_violin.png' ]);
                    end
%                 end
            end
            close all;
