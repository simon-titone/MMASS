...
%% Load Subjects
v.intersess_corr_NREM2 = [4 9 12 14 15 16 17 21]; 
v.intersess_corr_NREM3 = [4 9 10 12 14 15 16 17 21 28  22];
v.intersess_corr_REM   = [1 3 4 6 7 9 10 11 12 13 14 16 17 18 21 24 27 28 29]; 

v.group = v.intersess_corr_NREM2; stage = 1;
% v.group = v.intersess_corr_NREM3; stage = 2;
...
for st= stage
    for c= 1:v.ncycles
        for b = 1:v.nbands
            for w=1:v.nnetws
                for ww=1:v.nnetws
                    conn_vect1 = squeeze(intersess_NETW(w,ww,1,c,st,b,:));
                    conn_vect2 = squeeze(intersess_NETW(w,ww,2,c,st,b,:));

                    [val,px]=corr(conn_vect1,conn_vect2);
...
for c= 1:v.ncycles
    for st= stage
        ...
        for b=1:v.nbands
            ...
            colormap('redblue'); 
            limit = max(max(abs(tri_matrix))); 
            caxis([-limit limit]);
            
            if v.fig_labels ==1
            T = {['EXP corr CTRL connectivity'] [v.stages{st} ' ' v.bandname{b}  ', {\it n} = ' num2str(v.nsubj)] };
            else
            T = {['EXP corr CTRL connectivity'] [v.stages{st} ' ' v.bandname{b} ] };
            end
                ...
                for w=1:v.nnetws
                    for ww=1:v.nnetws
                        if p_val(w,ww,st,c,b) <0.05 == 1
                            ...
                            figure
                            x = squeeze(intersess_NETW(w,ww,1,c,st,b,:));
                            y = squeeze(intersess_NETW(w,ww,2,c,st,b,:));
                            ...
                            if v.fig_labels ==1
                                T_scatter = {[  'EXP corr CTRL connectivity ', '{\it n} = ' num2str(v.nsubj)] [v.stages{st} ' ' v.bandname{b} ' ' v.netwname{w} '-' v.netwname{ww} ] };
                            else
                                T_scatter = {[  'EXP corr CTRL connectivity ' ] [v.stages{st} ' ' v.bandname{b} ' ' v.netwname{w} '-' v.netwname{ww} ] };
                            end
