...
anova_dir = [v.homedir filesep 'results/ANOVA/interstage_all/'  ];
    ...
    for b = 1:v.nbands
        for w=1:v.nnetws
            for ww=1:v.nnetws
                ...
                vect1 = squeeze(NETW(w,ww,:,1,2,c,b));
                vect2 = squeeze(NETW(w,ww,:,2,2,c,b));
                vect3 = squeeze(NETW(w,ww,:,3,2,c,b));
                vect4 = squeeze(NETW(w,ww,:,4,2,c,b));
                nsubj = length(~isnan(vect1));
                mat(:,1) = vect1; mat(:,2) = vect2; mat(:,3) = vect3; mat(:,4) = vect4;
            ...
            for w=1:v.nnetws
                for ww=1:v.nnetws
                    if ANOVA_p(w,ww,c,b) <0.05 == 1
                        ...
                        x = [squeeze(NETW(w,ww,:,4,2,c,b)); squeeze(NETW(w,ww,:,1,2,c,b)); squeeze(NETW(w,ww,:,2,2,c,b)); squeeze(NETW(w,ww,:,3,2,c,b))];
                        y = [repmat({'RS'}, v.nsubj,1) ;repmat({'NREM2'}, v.nsubj,1) ; repmat({'NREM3'}, v.nsubj,1); repmat({'REM'}, v.nsubj,1)];
                        grouporder={'RS','NREM2', 'NREM3', 'REM'};
