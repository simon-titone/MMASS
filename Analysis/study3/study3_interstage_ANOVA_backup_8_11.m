...
v.stage_ANOVA = [4 9 12 14 16 17 ];                                    % n = 21
...
for se = 2
    for c= 1:v.ncycles
        for st= 1
            sleep_anova_dir_stage = [sleep_anova_dir  '/interstage/' v.stagediff{st}];
                    ...
                    for w=1:v.nnetws
                        for ww=1:v.nnetws
                            if ANOVA_p(w,ww,b,c,1) <0.05 == 1
                                ...
                                title({[v.sess{se} ' ' v.bandname{b}  ' Inter-stage ANOVA '] [ v.netwname{w} '-' v.netwname{ww} ', n= ' num2str(v.nsubj)]});
